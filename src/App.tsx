...
export default function App() {
  ...
  const [data, setData] = useState<any>(null); // Datos Pantalla 1 (Venezuela)
  const [globalRates, setGlobalRates] = useState<any[]>([]); // Datos Pantalla 2 (Mundial)
  ...
  const [activeTab, setActiveTab] = useState('home'); // Control de navegación
  const audioRef = useRef<HTMLAudioElement | null>(null);

  // --- 2. RELOJ Y TIEMPO REAL ---
  useEffect(() => {
    const updateTime = () => {
      const now = new Date();
  ...
  // --- 3. CONEXIÓN CON TU SERVIDOR (SUPABASE) ---
  const sync = useCallback(async () => {
    setLoading(true);
    try {
      // Petición A: Monitor Venezuela
  ...
  const handleShare = async () => {
    let text = activeTab === 'home' 
      ? `📊 *HOO Monitor*\n💵 Binance: ${data?.precio_usdt}\n🏦 BCV: ${data?.precio_bcv}`
      : `🌍 *Tasas Globales P2P*\nConsulta el mercado internacional en vivo.`;
    try { if (navigator.share) await navigator.share({ title: 'HOO Monitor', text, url: window.location.href }); } catch (err) {}
  };
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-[#050608] text-white overflow-hidden relative">
      ...
      <div className="marketing-banner"><div className="marketing-content">/// JAIROKOV SYSTEMS: SOLUCIONES DE IA A MEDIDA /// AUTOMATIZACIÓN ESTRATÉGICA DE PROCESOS /// GESTIÓN DE BASES DE DATOS ///</div></div>
