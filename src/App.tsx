...
export default function App() {
  ...
  // ==========================================
  // 3. SINCRONIZACIÓN (FIX PRECIO CONGELADO)
  // ==========================================
  const sync = useCallback(async () => {
    ...
    try {
      // Agregamos ?t=${Date.now()} para evitar que el navegador use datos viejos (caché)
      ...
      if (dbData && dbData.length > 0) {
        ...
        // Inyectamos el BCV real al final de las tasas globales
  ...
  useEffect(() => {
    if (isUnlocked) {
      sync();
      const interval = setInterval(sync, 30000); // Actualiza cada 30 seg
  ...
  return (
    <div className={`h-screen w-full flex flex-col bg-[#050608] text-white overflow-hidden relative font-sans transition-all duration-75 ${isGlitching ? 'opacity-40 brightness-150' : 'opacity-100'}`}>
      ...
      <div className="flex-1 flex flex-col container mx-auto max-w-lg px-6 py-4 no-scrollbar overflow-y-auto pb-32">
        ...
        {/* HEADER CON "H" DORADA Y SUBTITULO BETA */}
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'home' ? (
            <motion.div key="h" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              ...
              {/* MONITOR CON RELOJ 60s Y CÍRCULO DENTADO */}
              <div className="gauge-container relative flex items-center justify-center my-4 py-4">
                  ...
                  <svg width="220" height="220" viewBox="0 0 230 230" className="absolute"><circle cx="115" cy="115" r="102" fill="none" stroke="#1a1c1e" strokeWidth="14" /><circle cx="115" cy="115" r="102" fill="none" stroke={trendColor} strokeWidth="14" strokeLinecap="round" className={`transition-all duration-500 opacity-90`} transform='rotate(-90 115 115)' /></svg>
              ...
              <div className="my-4"><div className="border border-[#e2b053]/20 rounded-xl px-4 py-2 flex justify-between items-center bg-[#e2b053]/5"><div className="flex items-center gap-2 text-[8px] font-black text-white uppercase"><CalendarDays className="w-3 h-3 text-[#e2b053]" /> {vzlaTime.dayName}</div><div className="text-[8px] font-black text-white">{vzlaTime.date}</div><div className="flex items-center gap-2 text-[8px] font-black text-white"><Clock className="w-3 h-3 text-[#e2b053]" /> {vzlaTime.time}</div></div></div>
              
              <div className="grid grid-cols-2 gap-3 mt-4">
                <div className="border border-white/5 rounded-2xl p-4 bg-[#111214]"><span className="text-[9px] text-[#e2b053] uppercase font-black mb-1">Binance P2P</span><h3 className="font-rajdhani text-3xl font-bold">{data?.precio_usdt || '--,--'}</h3></div>
                
                {/* TENDENCIA ALINEADA IZQUIERDA CON FLECHAS */}
        ...
        {/* FOOTER CON HIPERLINK Y CONTRATACIONES */}
