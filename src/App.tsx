...
export default function App() {
  ...
  const sync = useCallback(async () => {
    ...
    try {
      ...
      if (d.id !== lastId) {
        // Verde si baja (fortalece Bs), Rojo si sube (devalúa)
      ...
      <div className="flex-1 flex flex-col container mx-auto max-w-lg px-6 py-4 no-scrollbar overflow-y-auto">
        <header className="flex justify-between items-start pt-2 mb-4">
            <div className="flex items-center gap-3">
                ...
                <div><h2 className="font-rajdhani text-xl font-bold tracking-widest leading-none">HOO</h2><p className="text-[7px] text-white/40 uppercase mt-1">DÓLAR MONITOR v6.5</p></div>
            </div>
        ...
        <div className="gauge-container relative flex items-center justify-center my-4 py-4">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-20">
                ...
                <span className="font-rajdhani text-4xl font-extrabold tracking-tighter leading-none">{data?.brecha_porcentaje || (currentGap.toFixed(2) + '%')}</span>
        ...
        <div className="flex flex-col gap-3 mt-4">
            <div className="grid grid-cols-2 gap-3">
                <div className="glass-card p-4 flex flex-col justify-center"><span className="text-[7px] text-white/30 uppercase tracking-widest mb-1">Binance P2P</span><h3 className="font-rajdhani text-3xl font-bold">{data?.precio_usdt || '--,--'}</h3><p className="text-[6px] text-white/20">VES / USDT</p></div>
                <div className="glass-card p-4 flex flex-col justify-between"><span className="text-[7px] text-white/30 uppercase tracking-widest mb-1">Tendencia</span><div className="flex flex-col items-end"><span className="font-rajdhani text-2xl font-black" style={{ color: trendColor }}>{data?.variacion_mercado || '0.00%'}</span><div className="h-5 w-full mt-2"><svg viewBox="0 0 100 25" className="w-full h-full"><path className="sparkline" style={{ stroke: trendColor }} d="M0,20 Q30,5 50,15 T100,0" /></svg></div></div></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
                <div className="glass-card p-3 flex justify-between items-center bg-[#181a1b]/80"><div><span className="text-[6px] text-white/40 uppercase tracking-[2px]">Euro BCV</span><span className="block font-rajdhani text-lg font-bold">€ {data?.precio_eur || '--,--'}</span></div><div className="text-[#e2b053] flex items-center gap-1 bg-[#e2b053]/10 px-2 py-1 rounded-md"><Landmark className="w-3 h-3" /><span className="text-[7px] font-black">BCV</span></div></div>
                <div className="glass-card p-3 flex justify-between items-center bg-[#181a1b]/80"><div><span className="text-[6px] text-white/40 uppercase tracking-[2px]">Dólar BCV</span><span className="block font-rajdhani text-lg font-bold">$ {data?.precio_bcv || '--,--'}</span></div><div className="text-[#e2b053] flex items-center gap-1 bg-[#e2b053]/10 px-2 py-1 rounded-md"><Landmark className="w-3 h-3" /><span className="text-[7px] font-black">BCV</span></div></div>
            </div>
        </div>

        <footer className="mt-auto pt-6"><div className="glass-card p-3 flex items-center justify-between border-white/5"><div><span className="text-[6px] text-white/30 uppercase">Sistema Elite</span><span className="block font-rajdhani text-xs font-bold tracking-[2px]">@JAIROKOV</span></div><div className="flex gap-4"><Instagram className="w-4 h-4 text-white/40" /><Send className="w-4 h-4 text-white/40" /><Twitter className="w-4 h-4 text-white/40" /></div></div><div className="text-center py-4"><p className="text-[7px] text-white/10 uppercase tracking-[0.4em]">Propiedad de Jairokov Systems © 2026</p></div></footer>
      </div>
    </div>
  );
}
